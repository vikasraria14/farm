...
const insertOrder = (id, username, productName, price, quantity, label, category) => {
  const sql = `
    INSERT INTO orders (id, username, product_name, price, quantity, order_status,  label, category)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `;

  const values = [id, username, productName, price, quantity, 'new', label, category];
...
const getOrdersBySeller = async (sellerId) => {
  const sql = `SELECT o.* FROM orders o JOIN product p ON o.product_name = p.name WHERE p.addedBy =? ORDER BY order_date DESC;`
